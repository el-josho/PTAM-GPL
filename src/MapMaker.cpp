...
MapMaker::MapMaker(Map& m, const ATANCamera& cam) : mMap(m), mCamera(cam) {
  ...
  Reset();
  start();  // This CVD::thread func starts the map-maker thread with function
            // run()
  GUI.RegisterCommand("SaveMap", GUICommandCallBack, this);
  GV3::Register(mgvdWiggleScale, "MapMaker.WiggleScale", 0.1,
...
void MapMaker::run() {
  ...
  while (!shouldStop())  // ShouldStop is a CVD::Thread func which return true
                         // if the thread is told to exit.
  {
    CHECK_RESET;
    ...
    // From here on, mapmaker does various map-maintenance jobs in a certain
    // priority
    // Hierarchy. For example, if there's a new key-frame to be added
    // (QueueSize() is >0)
    ...
    CHECK_RESET;
    // Are there any newly-made map points which need more measurements from
    // older key-frames?
    if (mbBundleConverged_Recent && QueueSize() == 0)
      ReFindNewlyMade();
    ...
    if (mbBundleConverged_Recent && mbBundleConverged_Full &&
        rand() % 20 == 0 && QueueSize() == 0)
    ...
    if (QueueSize() > 0)
      AddKeyFrameFromTopOfQueue();  // Integrate into map data struct, and
                                    // process
  }
}
...
bool MapMaker::AddPointEpipolar(KeyFrame& kSrc,
  ...
  if (v3RayEnd_TC[2] <= v3RayStart_TC[2])  // it's highly unlikely that we'll
                                           // manage to get anything out if
                                           // we're facing backwards wrt the
                                           // other camera's view-ray
    return false;
...
void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet,
  ...
  // then fixed.
  for (set<KeyFrame*>::iterator it = sAdjustSet.begin(); it != sAdjustSet.end();
       it++) {
  ...
  for (set<MapPoint*>::iterator it = sMapPoints.begin(); it != sMapPoints.end();
       it++) {
...
void MapMaker::GUICommandHandler(
    string sCommand,
