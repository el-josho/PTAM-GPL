...
class ATANCamera {
 public:
  ...
  // Image size get/set: updates the internal projection params to that target
  // image size.
  ...
  Vector<2> Project(const Vector<2>& camframe);  // Projects from camera z=1
                                                 // plane to pixel coordinates,
  ...
  // Useful for gvar-related reasons (in case some external func tries to read
  // the camera params gvar, and needs some defaults.)
  static const Vector<NUMTRACKERCAMPARAMETERS> mvDefaultParams;

 protected:
  GVars3::gvar3<Vector<NUMTRACKERCAMPARAMETERS> >
      mgvvCameraParams;  // The actual camera parameters
  ...
  double mdOnePixelDist;   // z=1 distance covered by a single pixel offset (a
                           // rough estimate!)
  ...
  double mdDistortionEnabled;  // One or zero depending on if distortion is on
                               // or off.
  ...
  // Radial distortion transformation factor: returns ration of distorted /
  // undistorted radius.
  ...
  friend class CameraCalibrator;  // friend declarations allow access to
                                  // calibration jacobian and camera update
                                  // function.
