...
// tmmintrin.h contains SSE3<> instrinsics, used for the ZMSSD search at the
// bottom..
...
PatchFinder::PatchFinder(int nPatchSize)
  ...
  int nMaxSSDPerPixel =
      500;  // Pretty arbitrary... could make a GVar out of this.
...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint& p,
  ...
  // Slightly dumb that we re-calculate this here when the tracker's already
  // done this!
  Vector<3> v3Cam = se3CFromW * p.v3WorldPos;
  double dOneOverCameraZ = 1.0 / v3Cam[2];
  // Project the source keyframe's one-pixel-right and one-pixel-down vectors
  ...
  mm2WarpInverse.T()[0] =
      m2CamDerivs * (v3MotionRight.slice<0, 2>() -
                     v3Cam.slice<0, 2>() * v3MotionRight[2] * dOneOverCameraZ) *
      dOneOverCameraZ;
  mm2WarpInverse.T()[1] =
      m2CamDerivs * (v3MotionDown.slice<0, 2>() -
                     v3Cam.slice<0, 2>() * v3MotionDown[2] * dOneOverCameraZ) *
      dOneOverCameraZ;
  double dDet = mm2WarpInverse[0][0] * mm2WarpInverse[1][1] -
                mm2WarpInverse[0][1] * mm2WarpInverse[1][0];
  mnSearchLevel = 0;

  // This warp matrix is likely not appropriate for finding at level zero, which
  // is
  ...
  // Some warps are inappropriate, e.g. too near the camera, too far, or
  // reflected,
...
// This is just a convenience function wich caluclates the warp matrix and
// generates
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint& p) {
  ...
  // Optimisation: Don't re-gen the coarse template if it's going to be
  // substantially the
  // same as was made last time. This saves time when the camera is not moving.
  // For this,
  // check that (a) this patchfinder is still working on the same map point and
  // (b) the
  ...
  for (int i = 0; !bNeedToRefreshTemplate && i < 2; i++) {
    ...
    const double dRefreshLimit =
        0.07;  // Sort of works out as half a pixel displacement in src img
  ...
  if (bNeedToRefreshTemplate) {
    int nOutside;  // Use CVD::transform to warp the patch according the the
                   // warping matrix m2
    // This returns the number of pixels outside the source image hit, which
    // should be zero.
    nOutside = CVD::transform(p.pPatchSourceKF->aLevels[p.nSourceLevel].im,
                              mimTemplate, m2, vec(p.irCenter), vec(mirCenter));
    ...
    // Store the parameters which allow us to determine if we need to
    // re-calculate
...
// This makes a template without warping. Used for epipolar search, where we
// don't really know
// what the warping matrix should be. (Although to be fair, I should do rotation
// for epipolar,
...
// the target keyframe to try and find the template. Looks only at FAST corner
// points
// which are within radius nRange of the center. (Params are supplied in Level0
// coords.) Returns true on patch found.
bool PatchFinder::FindPatchCoarse(ImageRef irPos,
                                  KeyFrame& kf,
                                  unsigned int nRange) {
  mbFound = false;
...
// Includes calculating image of derivatives (gradients.) The inverse
// composition
// used here operates on three variables: x offet, y offset, and difference in
// patch
...
void PatchFinder::MakeSubPixTemplate() {
  ...
  for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++)
    for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
      Vector<2> v2Grad;
      v2Grad[0] = 0.5 * (mimTemplate[ir + ImageRef(1, 0)] -
                         mimTemplate[ir - ImageRef(1, 0)]);
      v2Grad[1] = 0.5 * (mimTemplate[ir + ImageRef(0, 1)] -
                         mimTemplate[ir - ImageRef(0, 1)]);
      mimJacs[ir - ImageRef(1, 1)].first = v2Grad[0];
      mimJacs[ir - ImageRef(1, 1)].second = v2Grad[1];
      Vector<3> v3Grad =
          unproject(v2Grad);  // This adds the mean-difference jacobian..
  ...
  // if(nRank < 3)
  // cout << "BAD RANK IN MAKESUBPIXELTEMPLATE!!!!" << endl; // This does not
  // happen often (almost never!)

  mv2SubPixPos = mv2CoarsePos;  // Start the sub-pixel search at the result of
                                // the coarse search..
  mdMeanDiff = 0.0;
}
...
// Single iteration of inverse composition. This compares integral image
// positions in the
// template image to floating point positions in the target keyframe.
// Interpolation is
// bilinear, and performed manually (rather than using CVD::image_interpolate)
// since
// this is a special case where the mixing fractions for each pixel are
// identical.
double PatchFinder::IterateSubPix(KeyFrame& kf) {
  ...
  double dX =
      v2Base[0] - floor(v2Base[0]);  // Distances from pixel center of TL pixel
  ...
  unsigned long nRowOffset = &kf.aLevels[mnSearchLevel].im[ImageRef(0, 1)] -
                             &kf.aLevels[mnSearchLevel].im[ImageRef(0, 0)];
  for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
    pTopLeftPixel = &im[ ::ir(v2Base) +
                         ImageRef(1, ir.y)];  // n.b. the x=1 offset, as with y
    for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++) {
      float fPixel =  // Calc target interpolated pixel
          fMixTL * pTopLeftPixel[0] + fMixTR * pTopLeftPixel[1] +
          fMixBL * pTopLeftPixel[nRowOffset] +
...
// Calculate the Zero-mean SSD of the coarse patch and a target imate at a
// specific
// point.
