...
class TrackerData;
struct Trail  // This struct is used for initial correspondences of the first
              // stereo pair.
    {
  MiniPatch mPatch;
...
class Tracker {
 ...
 protected:
  ...
  void
  Reset();  // Restart from scratch. Also tells the mapmaker to reset itself.
  ...
  void TrackForInitialMap();  // This is called by TrackFrame if there is not a
                              // map yet.
  enum {
  ...
  void TrailTracking_Start();  // First frame of initial trail tracking. Called
                               // by TrackForInitialMap.
  int TrailTracking_Advance();  // Steady-state of initial trail tracking.
                                // Called by TrackForInitialMap.
  ...
  void
  AssessTrackingQuality();   // Heuristics to choose between good, poor, bad.
  void ApplyMotionModel();   // Decaying velocity motion model applied prior to
                             // TrackMap
  ...
  Vector<6> CalcPoseUpdate(
      std::vector<TrackerData*> vTD,
      double dOverrideSigma = 0.0,
      bool bMarkOutliers = false);  // Updates pose from found points.
  SE3<> mse3CamFromWorld;  // Camera pose: this is what the tracker updates
                           // every frame.
  ...
  void AddNewKeyFrame();  // Gives the current frame to the mapmaker to use as a
                          // keyframe
  ...
  bool
      mbJustRecoveredSoUseCoarse;  // Always use coarse tracking after recovery!
