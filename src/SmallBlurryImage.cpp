...
void SmallBlurryImage::MakeJacs() {
  ...
  do {
    ...
    if (mimTemplate.in_image_with_border(ir, 1)) {
      v2Grad[0] =
          mimTemplate[ir + ImageRef(1, 0)] - mimTemplate[ir - ImageRef(1, 0)];
      v2Grad[1] =
          mimTemplate[ir + ImageRef(0, 1)] - mimTemplate[ir - ImageRef(0, 1)];
...
pair<SE2<>, double> SmallBlurryImage::IteratePosRelToTarget(
  ...
  for (int it = 0; it < nIterations; it++) {
    ...
    do {
      ...
      here = imWarped[ir];
      if (l + r + u + d + here < -9999.9)  // This means it's out of the image;
                                           // c.f. the -9e20f param to
                                           // transform.
        continue;
      ...
      v4Jac[0] = v2SumGrad[0];
      v4Jac[1] = v2SumGrad[1];
      v4Jac[2] = -(ir.y - irCenter.y) * v2SumGrad[0] +
                 (ir.x - irCenter.x) * v2SumGrad[1];
      //	  v4Jac[3] = 1.0;
...
SE3<> SmallBlurryImage::SE3fromSE2(SE2<> se2, ATANCamera camera) {
  ...
  av3OrigPoints[0] =
      unproject(camera.UnProject(vec(mirSize / 2) + vec(ImageRef(5, 0))));
  av3OrigPoints[1] =
      unproject(camera.UnProject(vec(mirSize / 2) + vec(ImageRef(-5, 0))));
  ...
  for (int it = 0; it < 3; it++) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      for (int m = 0; m < 3; m++) {
        ...
        v2CamFrameMotion[0] =
            (v3Motion[0] - v3Cam[0] * v3Motion[2] * dOneOverCameraZ) *
            dOneOverCameraZ;
        v2CamFrameMotion[1] =
            (v3Motion[1] - v3Cam[1] * v3Motion[2] * dOneOverCameraZ) *
            dOneOverCameraZ;
