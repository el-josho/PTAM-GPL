...
inline bool IsCorner(Image<byte>& im,
                     ImageRef ir,
                     int nGate) {  // Does a quick check to see if a point in an
                                   // image could be a grid corner.
...
double CalibGridCorner::ExpansionPotential() {
  ...
  // The best case is if it's already surrounded by three neighbors and only
  // needs
...
void CalibImage::ExpandByStep(int n) {
  static gvar3<double> gvdMaxStepDistFraction(
      "CameraCalibrator.ExpandByStepMaxDistFrac", 0.3, SILENT);
  static gvar3<int> gvnCornerPatchSize("CameraCalibrator.CornerPatchPixelSize",
                                       20, SILENT);
  ...
  for (int i = 0; i < (int)mvGridCorners.size(); i++) {
    for (int dirn = 0; dirn < 4; dirn++)
      if (mvGridCorners[i].aNeighborStates[dirn].val > i) {
        ...
        glVertex(mvGridCorners[mvGridCorners[i].aNeighborStates[dirn].val]
                     .Params.v2Pos);
      }
  }
  glEnd();
...
void CalibImage::Draw3DGrid(ATANCamera& Camera, bool bDrawErrors) {
  ...
  for (int i = 0; i < (int)mvGridCorners.size(); i++) {
    for (int dirn = 0; dirn < 4; dirn++)
      if (mvGridCorners[i].aNeighborStates[dirn].val > i) {
        ...
        v3.slice<0, 2>() =
            vec(mvGridCorners[mvGridCorners[i].aNeighborStates[dirn].val]
                    .irGridPos);
        glVertex(Camera.Project(project(mse3CamFromWorld * v3)));
      }
  }
  ...
  if (bDrawErrors) {
    ...
    for (int i = 0; i < (int)mvGridCorners.size(); i++) {
      ...
      v3[2] = 0.0;
      v3.slice<0, 2>() = vec(mvGridCorners[i].irGridPos);
      Vector<2> v2Pixels_Projected =
          Camera.Project(project(mse3CamFromWorld * v3));
...
void CalibImage::GuessInitialPose(ATANCamera& Camera) {
  // First, find a homography which maps the grid to the unprojected image
  // coords
  ...
  // The right null-space (should only be one) of the matrix gives the
  // homography...
  ...
  // Fix up possibly poorly conditioned bits of the homography
  {
    ...
    Vector<2>
        v2b;  // This is one hypothesis for v2b ; the other is the negative.
    v2b[0] = 0.0;
  ...
  mRotation.T()[1] =
      m3Homography.T()[1] -
      m3Homography.T()[0] * (m3Homography.T()[0] * m3Homography.T()[1]);
...
vector<CalibImage::ErrorAndJacobians> CalibImage::Project(ATANCamera& Camera) {
  ...
  for (unsigned int n = 0; n < mvGridCorners.size(); n++) {
    ...
    for (int dof = 0; dof < 6; dof++) {
      ...
      v2CamFrameMotion[0] =
          (v4Motion[0] - v3Cam[0] * v4Motion[2] * dOneOverCameraZ) *
          dOneOverCameraZ;
      v2CamFrameMotion[1] =
          (v4Motion[1] - v3Cam[1] * v4Motion[2] * dOneOverCameraZ) *
          dOneOverCameraZ;
