...
Bundle::Bundle(const ATANCamera& TCam) : mCamera(TCam) {
  ...
  GV3::Register(mgvdUpdateConvergenceLimit,
                "Bundle.UpdateSquaredConvergenceLimit", 1e-06, SILENT);
  GV3::Register(mgvnBundleCout, "Bundle.Cout", 0, SILENT);
};

// Add a camera to the system, return value is the bundle adjuster's ID for the
// camera
...
// Add a map point to the system, return value is the bundle adjuster's ID for
// the point
...
int Bundle::Compute(bool* pbAbortSignal) {
  ...
  cout << "Final Sigma Squared: " << mdSigmaSquared
       << " (= " << sqrt(mdSigmaSquared) / 4.685 << " pixels.)" << endl;
  return mnAccepted;
};
...
bool Bundle::Do_LM_Step(bool* pbAbortSignal) {
  ...
  //  Do a LM step according to Hartley and Zisserman Algo A6.4 in MVG 2nd
  //  Edition
  ...
  for (list<Meas>::iterator itr = mMeasList.begin(); itr != mMeasList.end();
       itr++) {
    Meas& meas = *itr;
  ...
  for (list<Meas>::iterator itr = mMeasList.begin(); itr != mMeasList.end();
    ...
    // What to do with the weights? The easiest option is to independently
    // weight
    ...
    double dWeight =
        MEstimator::SquareRootWeight(meas.dErrorSquared, mdSigmaSquared);
    // Re-weight error:
    ...
    dCurrentError +=
        MEstimator::ObjectiveScore(meas.dErrorSquared, mdSigmaSquared);
    ...
    if (cam.bFixed)
      ...
      for (int m = 0; m < 6; m++) {
        ...
        v2CamFrameMotion[0] =
            (v4Motion[0] - v4Cam[0] * v4Motion[2] * dOneOverCameraZ) *
            dOneOverCameraZ;
        v2CamFrameMotion[1] =
            (v4Motion[1] - v4Cam[1] * v4Motion[2] * dOneOverCameraZ) *
            dOneOverCameraZ;
    ...
    for (int m = 0; m < 3; m++) {
      ...
      Vector<2> v2CamFrameMotion;
      v2CamFrameMotion[0] =
          (v3Motion[0] - v4Cam[0] * v3Motion[2] * dOneOverCameraZ) *
          dOneOverCameraZ;
      v2CamFrameMotion[1] =
          (v3Motion[1] - v4Cam[1] * v3Motion[2] * dOneOverCameraZ) *
          dOneOverCameraZ;
    ...
    if (!cam.bFixed) {
      //	  cam.m6U += meas.m26A.T() * meas.m26A; 	  // SLOW SLOW
    ...
    //            point.m3V += meas.m23B.T() * meas.m23B;             //
    //            SLOW-ish this is symmetric too
  ...
  // OK, done (i) and most of (ii) except calcing Yij; this depends on Vi, which
  // should
  // be finished now. So we can find V*i (by adding lambda) and then invert.
  // The next bits depend on mdLambda! So loop this next bit until error goes
  // down.
  double dNewError = dCurrentError + 9999;
  while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations &&
         !*pbAbortSignal) {
    // Rest of part (ii) : find V*i inverse
    for (vector<Point>::iterator itr = mvPoints.begin(); itr != mvPoints.end();
         itr++) {
      Point& point = *itr;
    ...
    for (unsigned int j = 0; j < mvCameras.size(); j++) {
      ...
      for (unsigned int i = 0; i < mvPoints.size(); i++) {
        ...
        if (pMeas == NULL || pMeas->bBad)
          continue;
        m6 -= pMeas->m63W * mvPoints[i].m3VStarInv *
              pMeas->m63W.T();  // SLOW SLOW should by 6x6sy
    ...
    // Now find off-diag elements of S. These are camera-point-camera
    // combinations, of which there are lots.
    // New code which doesn't waste as much time finding i-jk pairs; these are
    // pre-stored in a per-i list.
    for (unsigned int i = 0; i < mvPoints.size(); i++) {
      Point& p = mvPoints[i];
      int nCurrentJ = -1;
      int nJRow = -1;
      ...
      for (vector<OffDiagScriptEntry>::iterator it =
               p.vOffDiagonalScript.begin();
           it != p.vOffDiagonalScript.end(); it++) {
...
#ifndef WIN32
        mS.slice(nJRow, nKRow, 6, 6) -=
            m63_MIJW_times_m3VStarInv * pMeas_ik->m63W.T();
#else
    ...
    for (unsigned int i = 0; i < mvPoints.size(); i++) {
      ...
      if (std::isnan(vMapUpdates.slice(i * 3, 3) *
                     vMapUpdates.slice(i * 3, 3))) {
        cerr << "NANNERY! " << endl;
    ...
    double dSumSquaredUpdate =
        vCamerasUpdate * vCamerasUpdate + vMapUpdates * vMapUpdates;
    ...
    for (unsigned int j = 0; j < mvCameras.size(); j++) {
      if (mvCameras[j].bFixed)
        ...
        mvCameras[j].se3CfWNew =
            SE3<>::exp(vCamerasUpdate.slice(mvCameras[j].nStartRow, 6)) *
            mvCameras[j].se3CfW;
    }
    for (unsigned int i = 0; i < mvPoints.size(); i++)
    ...
    cout << setprecision(1) << "L" << mdLambda << setprecision(3) << "\tOld "
         << dCurrentError << "  New " << dNewError << "  Diff "
         << dCurrentError - dNewError << "\t";
...
void Bundle::GenerateOffDiagScripts() {
  for (unsigned int i = 0; i < mvPoints.size(); i++) {
    ...
    for (set<int>::iterator it_j = p.sCameras.begin(); it_j != p.sCameras.end();
         it_j++) {
      int j = *it_j;
      if (mvCameras[j].bFixed)
        continue;
