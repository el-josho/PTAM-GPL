...
MapViewer::MapViewer(Map& map, GLWindow2& glw) : mMap(map), mGLWindow(glw) {
  mse3ViewerFromWorld = SE3<>::exp(makeVector(0, 0, 2, 0, 0, 0)) *
...
void MapViewer::DrawMap(SE3<> se3CamFromWorld) {
  ...
  // Update viewer position according to mouse input:
  {
    ...
    mse3ViewerFromWorld = SE3<>::exp(pv6.first) * se3CamFromMC *
                          SE3<>::exp(pv6.second) * se3CamFromMC.inverse() *
                          mse3ViewerFromWorld;
  }
  ...
  mMessageForUser << "   Camera Pos: "
                  << se3CamFromWorld.inverse().get_translation();
}
