...
void GLWindow2::GUICommandHandler(
    string sCommand,
    string sParams)  // Called by the callback func..
{
  ...
  if (sCommand == "GLWindow.AddMenu") {
    switch (vs.size()) {
      ...
      case 2:
        ...
        return;
      default:
        cout << "? AddMenu: need one or two params (internal menu name, "
                "[caption])." << endl;
        return;
    };
  };
  ...
  cout << "! GLWindow::GUICommandHandler: unhandled command " << sCommand
       << endl;
  exit(1);
};
...
void GLWindow2::SetupVideoOrtho() {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glOrtho(-0.5, (double)mirVideoSize.x - 0.5, (double)mirVideoSize.y - 0.5,
          -0.5, -1.0, 1.0);
}
...
void GLWindow2::on_mouse_move(GLWindow& win, CVD::ImageRef where, int state) {
  ...
  } else if (state & BUTTON_MIDDLE ||
             (state & BUTTON_LEFT && state & BUTTON_RIGHT)) {
...
void GLWindow2::on_mouse_down(GLWindow& win,
  ...
  for (unsigned int i = 0; !bHandled && i < mvpGLWindowMenus.size(); i++)
    bHandled =
        mvpGLWindowMenus[i]->HandleClick(button, state, where.x, where.y);
}
...
pair<Vector<6>, Vector<6> > GLWindow2::GetMousePoseUpdate() {
  pair<Vector<6>, Vector<6> > result =
      make_pair(mvLeftPoseUpdate, mvMCPoseUpdate);
  mvLeftPoseUpdate = Zeros;
  mvMCPoseUpdate = Zeros;
  return result;
}
