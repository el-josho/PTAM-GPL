...
System::System() : mGLWindow(mVideoSource.Size(), "PTAM") {
  ...
  vTest = GV3::get<Vector<NUMTRACKERCAMPARAMETERS> >(
      "Camera.Parameters", ATANCamera::mvDefaultParams, HIDDEN);
  ...
  if (vTest == ATANCamera::mvDefaultParams) {
    cout << endl;
    cout << "! Camera.Parameters is not set, need to run the CameraCalibrator "
            "tool" << endl;
    cout << "  and/or put the Camera.Parameters= line into the appropriate "
            ".cfg file." << endl;
    exit(1);
  }
