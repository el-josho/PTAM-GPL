...
Tracker::Tracker(ImageRef irVideoSize,
                 const ATANCamera& c,
                 Map& m,
                 MapMaker& mm)
    : mMap(m),
...
// This is the main Reset-handler-entry-point of the program! Other classes'
// resets propagate from here.
...
// It figures out what state the tracker is in, and calls appropriate internal
// tracking
...
void Tracker::TrackFrame(Image<byte>& imFrame, bool bDraw) {
  ...
  // Take the input video image, and convert it into the tracker's keyframe
  // struct
  ...
  static gvar3<double> gvdSBIBlur("Tracker.RotationEstimatorBlur", 0.75,
                                  SILENT);
    ...
    if (mnLostFrames < 3)  // .. but only if we're not lost!
    {
      ...
      {  // Provide some feedback for the user:
        ...
        for (int i = 0; i < LEVELS; i++)
          mMessageForUser << " " << manMeasFound[i] << "/"
                          << manMeasAttempted[i];
        //	    mMessageForUser << " Found " << mnMeasFound << " of " <<
        // mnMeasAttempted <<". (";
      ...
      if (mTrackingQuality == GOOD && mMapMaker.NeedNewKeyFrame(mCurrentKF) &&
  ...
  while (!mvQueuedCommands.empty()) {
    GUICommandHandler(mvQueuedCommands.begin()->sCommand,
                      mvQueuedCommands.begin()->sParams);
...
// Draw the reference grid to give the user an idea of wether tracking is OK or
// not.
void Tracker::RenderGrid() {
  ...
  // The grid is projected manually, i.e. GL receives projected 2D coords to
  // draw.
  int nHalfCells = 8;
...
void Tracker::GUICommandHandler(
    string sCommand,
    string sParams)  // Called by the callback func..
{
  if (sCommand == "Reset") {
    Reset();
    return;
  ...
  cout << "! Tracker::GUICommandHandler: unhandled command " << sCommand
       << endl;
  exit(1);
};

// Routine for establishing the initial map. This requires two spacebar presses
// from the user
// to define the first two key-frames. Salient points are tracked between the
// two keyframes
// using cheap frame-to-frame tracking (which is very brittle - quick camera
// motion will
// break it.) The salient points are stored in a list of `Trail' data
// structures.
// What action TrackForInitialMap() takes depends on the mnInitialStage enum
// variable..
void Tracker::TrackForInitialMap() {
  ...
  if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED) {
    if (mbUserPressedSpacebar)  // First spacebar = this is the first keyframe
    {
      ...
      mMessageForUser << "Point camera at planar scene and press spacebar to "
                         "start tracking for initial map." << endl;
    return;
  };

  if (mnInitialStage == TRAIL_TRACKING_STARTED) {
    int nGoodTrails =
        TrailTracking_Advance();  // This call actually tracks the trails
    if (nGoodTrails <
        10)  // if most trails have been wiped out, no point continuing.
    {
      Reset();
      return;
    }

    // If the user pressed spacebar here, use trails to run stereo and make the
    // intial map..
    if (mbUserPressedSpacebar) {
      mbUserPressedSpacebar = false;
      vector<pair<ImageRef, ImageRef> > vMatches;  // This is the format the
                                                   // mapmaker wants for the
                                                   // stereo pairs
      for (list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end(); i++)
        vMatches.push_back(
            pair<ImageRef, ImageRef>(i->irInitialPos, i->irCurrentPos));
      mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches,
                               mse3CamFromWorld);  // This will take some time!
      mnInitialStage = TRAIL_TRACKING_COMPLETE;
    } else
      mMessageForUser << "Translate the camera slowly sideways, and press "
                         "spacebar again to perform stereo init." << endl;
  }
}

// The current frame is to be the first keyframe!
void Tracker::TrailTracking_Start() {
  mCurrentKF.MakeKeyFrame_Rest();  // This populates the Candidates list, which
                                   // is Shi-Tomasi thresholded.
  mFirstKF = mCurrentKF;
  vector<pair<double, ImageRef> > vCornersAndSTScores;
  for (unsigned int i = 0; i < mCurrentKF.aLevels[0].vCandidates.size();
       i++)  // Copy candidates into a trivially sortable vector
  {          // so that we can choose the image corners with max ST score
    Candidate& c = mCurrentKF.aLevels[0].vCandidates[i];
    if (!mCurrentKF.aLevels[0].im.in_image_with_border(
            c.irLevelPos, MiniPatch::mnHalfPatchSize))
      continue;
    vCornersAndSTScores.push_back(pair<double, ImageRef>(
        -1.0 * c.dSTScore,
  ...
  for (unsigned int i = 0; i < vCornersAndSTScores.size() && nToAdd > 0; i++) {
    ...
    t.mPatch.SampleFromImage(vCornersAndSTScores[i].second,
                             mCurrentKF.aLevels[0].im);
    t.irInitialPos = vCornersAndSTScores[i].second;
    t.irCurrentPos = t.irInitialPos;
    mlTrails.push_back(t);
    nToAdd--;
  }
  mPreviousFrameKF = mFirstKF;  // Always store the previous frame so
                                // married-matching can work.
}
...
// It first projects all map points into the image to find a
...
// class TrackerData handles the projection of a MapPoint and stores
...
void Tracker::TrackMap() {
  ...
  for (unsigned int i = 0; i < mMap.vpPoints.size(); i++) {
    ...
    // And check what the PatchFinder (included in TrackerData) makes of the
    // mappoint in this view..
    TData.nSearchLevel = TData.Finder.CalcSearchLevelAndWarpMatrix(
        TData.Point, mse3CamFromWorld, TData.m2CamDerivs);
    if (TData.nSearchLevel == -1)
      continue;  // a negative search pyramid level indicates an inappropriate
                 // warp for this view, so skip.

    // Otherwise, this point is suitable to be searched in the current image!
    // Add to the PVS.
  ...
  // Next: A large degree of faffing about and deciding which points are going
  // to be measured!
  ...
  if (*gvnCoarseDisabled || mdMSDScaledVelocityMagnitude < *gvdCoarseMinVel ||
      nCoarseMax == 0)
    bTryCoarse = false;
  ...
  // If we do want to do a coarse stage, also check that there's enough
  // high-level
  // PV map points. We use the lowest-res two pyramid levels (LEVELS-1 and
  // LEVELS-2),
  ...
  if (bTryCoarse &&
    ...
    // there are in different pyramid levels compared to CoarseMin and
    // CoarseMax.

    if (avPVS[LEVELS - 1].size() <=
        nCoarseMax) {  // Fewer than CoarseMax in LEVELS-1? then take all of
                       // them, and remove them from the PVS list.
      vNextToSearch = avPVS[LEVELS - 1];
      avPVS[LEVELS - 1].clear();
    } else {  // ..otherwise choose nCoarseMax at random, again removing from
              // the PVS list.
    ...
    // If didn't source enough from LEVELS-1, get some from LEVELS-2... same as
    // above.
    ...
    if (nFound >=
        *gvnCoarseMin)  // Were enough found to do any meaningful optimisation?
    {
      mbDidCoarse = true;
      ...
      {
        if (iter !=
            0) {  // Re-project the points on all but the first iteration.
  ...
  if (mbDidCoarse)  // Can use a tighter search if the coarse stage was already
                    // done.
    nFineRange = 5;

  // What patches shall we use this time? The high-level ones are quite
  // important,
  // so do all of these, with sub-pixel refinement.
  {
    ...
    for (unsigned int i = 0; i < avPVS[l].size(); i++)
      vIterationSet.push_back(avPVS[l][i]);  // Again, plonk all searched points
                                             // onto the (maybe already
                                             // populate) vIterationSet.
  };

  // All the others levels: Initially, put all remaining potentially visible
  // patches onto vNextToSearch.
  vNextToSearch.clear();
  for (int l = LEVELS - 2; l >= 0; l--)
    for (unsigned int i = 0; i < avPVS[l].size(); i++)
      vNextToSearch.push_back(avPVS[l][i]);

  // But we haven't got CPU to track _all_ patches in the map - arbitrarily
  // limit
  // ourselves to 1000, and choose these randomly.
  static gvar3<int> gvnMaxPatchesPerFrame("Tracker.MaxPatchesPerFrame", 1000,
                                          SILENT);
  ...
  // If we did a coarse tracking stage: re-project and find derivs of fine
  // points
  if (mbDidCoarse)
  ...
  for (int iter = 0; iter < 10; iter++) {
    ...
    // Calculate and update pose; also store update vector for linear iteration
    // updates.
    Vector<6> v6Update =
  ...
  // Record successful measurements. Use the KeyFrame-Measurement struct for
  // this.
  mCurrentKF.mMeasurements.clear();
  ...
  // Finally, find the mean scene depth from tracked features
  {
    ...
    if (nNum > 20) {
      ...
      mCurrentKF.dSceneDepthSigma =
          sqrt((dSumSq / nNum) -
               (mCurrentKF.dSceneDepthMean) * (mCurrentKF.dSceneDepthMean));
    }
  }
}

// Find points in the image. Uses the PatchFiner struct stored in TrackerData
int Tracker::SearchForPoints(vector<TrackerData*>& vTD,
                             int nRange,
                             int nSubPixIts) {
  int nFound = 0;
  for (unsigned int i = 0; i < vTD.size(); i++)  // for each point..
  {
    ...
    manMeasAttempted[Finder.GetLevel()]++;  // Stats for tracking quality
                                            // assessmenta
    ...
    if (nSubPixIts > 0) {
      ...
      bool bSubPixConverges =
          Finder.IterateSubPixToConvergence(mCurrentKF, nSubPixIts);
      if (!bSubPixConverges) {  // If subpix doesn't converge, the patch
                                // location is probably very dubious!
        TD.bFound = false;
        nFound--;
        manMeasFound[Finder.GetLevel()]--;
        continue;
...
Vector<6> Tracker::CalcPoseUpdate(vector<TrackerData*> vTD,
  ...
  // Also, store the square of these quantities for M-Estimator sigma squared
  // estimation.
  ...
  if (dOverrideSigma > 0)
    dSigmaSquared = dOverrideSigma;  // Bit of a waste having stored the vector
                                     // of square errors in this case!
  else {
    if (nEstimator == 0)
  ...
  for (unsigned int f = 0; f < vTD.size(); f++) {
    ...
    // Inlier/outlier accounting, only really works for cut-off estimators such
    // as Tukey.
    if (dWeight == 0.0) {
      if (bMarkOutliers)
    ...
    wls.add_mJ(v2[1], TD.dSqrtInvNoise * m26Jac[1],
               dWeight);  // the slowest bit of poseits
  }
...
void Tracker::UpdateMotionModel() {
  ...
  // Also make an estimate of this which has been scaled by the mean scene
  // depth.
...
void Tracker::AssessTrackingQuality() {
  ...
  if (nTotalFound == 0 || nTotalAttempted == 0)
    ...
    static gvar3<double> gvdQualityGood("Tracker.TrackingQualityGood", 0.3,
                                        SILENT);
    static gvar3<double> gvdQualityLost("Tracker.TrackingQualityLost", 0.13,
                                        SILENT);
