...
GLWindowMenu::GLWindowMenu(string sName, string sTitle) {
  ...
  GV2.Register(mgvnMenuItemWidth, msName + ".MenuItemWidth", 90,
               HIDDEN | SILENT);
  GV2.Register(mgvnMenuTextOffset, msName + ".MenuTextOffset", 20,
               HIDDEN | SILENT);
...
void GLWindowMenu::GUICommandHandler(string sCommand, string sParams) {
  ...
  if (sCommand == msName + ".AddMenuButton") {
    if (vs.size() < 3) {
      cout << "? GLWindowMenu.AddMenuButton: Need 3/4 params: Target Menu, "
              "Name, Command , NextMenu=\"\"" << endl;
      return;
    };
    MenuItem m;
    m.type = Button;
  ...
  if (sCommand == msName + ".AddMenuToggle") {
    if (vs.size() < 3) {
      cout << "? GLWindowMenu.AddMenuToggle: Need 3/4 params: Target Menu, "
              "Name, gvar_int name , NextMenu=\"\"" << endl;
      return;
    };
    MenuItem m;
    m.type = Toggle;
  ...
  if (sCommand == msName + ".AddMenuMonitor") {
    if (vs.size() < 3) {
      cout << "? GLWindowMenu.AddMenuMonitor: Need 3/4 params: Target Menu, "
              "Name, gvar name , NextMenu=\"\"" << endl;
      return;
    };
    MenuItem m;
    m.type = Monitor;
  ...
  if (sCommand == msName + ".AddMenuSlider") {
    if (vs.size() < 5) {
      cout << "? GLWindowMenu.AddMenuSlider: Need 5/6 params: Target Menu, "
              "Name, gvar_int name, min, max, NextMenu=\"\"" << endl;
      return;
    };
    MenuItem m;
    m.type = Slider;
...
void GLWindowMenu::Render(int nTop, int nHeight, int nWidth, GLWindow2& glw) {
  ...
  for (vector<MenuItem>::reverse_iterator i = m.mvItems.rbegin();
    ...
    switch (i->type) {
      case Button:
        ...
        FillBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
                mnMenuTop + mnMenuHeight);
        glColor4d(0, 1, 0, dAlpha);
        LineBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
                mnMenuTop + mnMenuHeight);
      ...
      case Toggle:
        ...
        LineBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
                mnMenuTop + mnMenuHeight);
        glw.PrintString(
            ImageRef(nBase + 3, mnMenuTop + *mgvnMenuTextOffset),
      ...
      case Slider: {
        ...
        if (dFrac > 1.0)
          dFrac = 1.0;
        FillBox(nBase, (int)(nBase + dFrac * (*mgvnMenuItemWidth + 1)),
                mnMenuTop, mnMenuTop + mnMenuHeight);
        glColor4d(0, 1, 1, dAlpha);
        LineBox(nBase, nBase + *mgvnMenuItemWidth + 1, mnMenuTop,
                mnMenuTop + mnMenuHeight);
        ostringstream ost;
  ...
  FillBox(mnWidth - *mgvnMenuItemWidth, mnWidth - 1, mnMenuTop,
          mnMenuTop + mnMenuHeight);
  glColor4d(1, 1, 0, dAlpha);
  LineBox(mnWidth - *mgvnMenuItemWidth, mnWidth - 1, mnMenuTop,
          mnMenuTop + mnMenuHeight);
  ImageRef ir(mnWidth - *mgvnMenuItemWidth + 5,
              mnMenuTop + *mgvnMenuTextOffset);
...
bool GLWindowMenu::HandleClick(int nMouseButton, int state, int x, int y) {
  ...
  MenuItem SelectedItem =
      mmSubMenus[msCurrentSubMenu].mvItems[nButtonNumber - 1];
    ...
    case Slider: {
      if (nMouseButton == GLWindow::BUTTON_WHEEL_UP) {
        ...
        *(SelectedItem.gvnIntValue) =
            (int)(dFrac * (1.0 + SelectedItem.max - SelectedItem.min)) +
            SelectedItem.min;
      };
    } break;
    case Monitor:
