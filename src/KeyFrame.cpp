...
void KeyFrame::MakeKeyFrame_Lite(BasicImage<byte>& im) {
  ...
  // detection, etc.
  // Does not fully populate the keyframe struct, but only does the bits needed
  // for the tracker;
  // e.g. does not perform FAST nonmax suppression. Things like that which are
  // needed by the
...
void KeyFrame::MakeKeyFrame_Rest() {
  ...
  // FAST nonmax suppression, generation of the list of candidates for further
  // map points,
  // creation of the relocaliser's SmallBlurryImage.
  static gvar3<double> gvdCandidateMinSTScore(
      "MapMaker.CandidateMinShiTomasiScore", 70, SILENT);
  ...
  for (int l = 0; l < LEVELS; l++) {
    ...
    // .. and then calculate the Shi-Tomasi scores of those, and keep the ones
    // with
    // a suitably high score as Candidates, i.e. points which the mapmaker will
    // attempt
    // to make new map points out of.
    for (vector<ImageRef>::iterator i = lev.vMaxCorners.begin();
         i != lev.vMaxCorners.end(); i++) {
...
// The keyframe struct is quite happy with default operator=, but Level needs
// its own
// to override CVD's reference-counting behaviour.
Level& Level::operator=(const Level& rhs) {
  // Operator= should physically copy pixels, not use CVD's reference-counting
  // image copy.
...
// These globals are filled in here. A single static instance of this struct is
// run before main()
struct LevelHelpersFiller  // Code which should be initialised on init goes
                           // here; this runs before main()
    {
  LevelHelpersFiller() {
    for (int i = 0; i < LEVELS; i++) {
      if (i == 0)
        ...
        gavLevelColors[i] = makeVector(0.0, 0.0, 0.7);
      else
        gavLevelColors[i] =
            makeVector(1.0, 1.0, 0.7);  // In case I ever run with LEVELS > 4
    }
  }
};
