...
bool HomographyInit::IsHomographyInlier(Matrix<3> m3Homography,
  ...
  Vector<2> v2Projected =
      project(m3Homography * unproject(match.v2CamPlaneFirst));
...
bool HomographyInit::Compute(vector<HomographyMatch> vMatches,
  ...
  // At this stage should have eight decomposition options, if all went
  // according to plan
...
Matrix<3> HomographyInit::HomographyFromMatches(
...
void HomographyInit::RefineHomographyWithInliers() {
  ...
  for (unsigned int i = 0; i < mvHomographyInliers.size(); i++) {
    ...
    Vector<2> v2Error = mvHomographyInliers[i].m2PixelProjectionJac *
                        (v2Second_real - v2Second);
    ...
    m29Jacobian[1].slice<6, 3>() =
        -unproject(v2First) * dNumerator / (dDenominator * dDenominator);

    vmJacobians.push_back(mvHomographyInliers[i].m2PixelProjectionJac *
                          m29Jacobian);
  }
...
void HomographyInit::BestHomographyFromMatches_MLESAC() {
  // Not many matches? Don't do ransac, throw them all in a pot and see what
  // comes out.
...
void HomographyInit::DecomposeHomography() {
  ...
  double d1 = fabs(v3Diag[0]);  // The paper suggests the square of these (e.g.
                                // the evalues of AAT)
  double d2 = fabs(
      v3Diag[1]);  // should be used, but this is wrong. c.f. Faugeras' book.
  ...
  if (nCase != 1) {
    cout << "  Homographyinit: This motion case is not implemented or is "
            "degenerate. Try again. " << endl;
    return;
  }
  ...
  // While we have the SVD results calculated here, store the decomposition R
  // and t results as well..
...
static double SampsonusError(Vector<2>& v2Dash,
  ...
  return (dError * dError /
          (fv3Slice * fv3Slice + fTv3DashSlice * fTv3DashSlice));
}
...
void HomographyInit::ChooseBestDecomposition() {
  ...
  for (unsigned int i = 0; i < mvDecompositions.size(); i++) {
    ...
    for (unsigned int m = 0; m < mvHomographyInliers.size(); m++) {
      ...
      double dVisibilityTest =
          (mm3BestHomography[2][0] * v2[0] + mm3BestHomography[2][1] * v2[1] +
           mm3BestHomography[2][2]) /
          decom.d;
  ...
  double dRatio =
      (double)mvDecompositions[1].nScore / (double)mvDecompositions[0].nScore;
  ...
  else  // two-way ambiguity. Resolve by sampsonus score of all points.
  {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      for (int j = 0; j < 3; j++)
        m3Essential.T()[j] =
            se3.get_translation() ^ se3.get_rotation().get_matrix().T()[j];

      double dSumError = 0;
      for (unsigned int m = 0; m < mvMatches.size(); m++) {
        double d = SampsonusError(mvMatches[m].v2CamPlaneSecond, m3Essential,
                                  mvMatches[m].v2CamPlaneFirst);
