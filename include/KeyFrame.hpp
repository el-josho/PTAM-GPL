...
// Measurement: A 2D image measurement of a map point. Each keyframe stores a
// bunch of these.
struct Measurement {
...
struct Level {
  ...
  bool bImplaneCornersCached;  // Also keep image-plane (z=1) positions of FAST
                               // corners to speed up epipolar search
  std::vector<Vector<2> >
      vImplaneCorners;  // Corner points un-projected into z=1-plane coordinates
};

// The actual KeyFrame struct. The map contains of a bunch of these. However,
// the tracker uses this
// struct as well: every incoming frame is turned into a keyframe before
// tracking; most of these
// are then simply discarded, but sometimes they're then just added to the map.
struct KeyFrame {
  inline KeyFrame() { pSBI = NULL; }
  SE3<> se3CfromW;  // The coordinate frame of this key-frame as a
                    // Camera-From-World transformation
  bool bFixed;  // Is the coordinate frame of this keyframe fixed? (only true
                // for first KF!)
  Level aLevels[LEVELS];  // Images, corners, etc lives in this array of pyramid
                          // levels
  std::map<MapPoint*, Measurement>
      mMeasurements;  // All the measurements associated with the keyframe

  void MakeKeyFrame_Lite(CVD::BasicImage<CVD::byte>& im);  // This takes an
                                                           // image and
                                                           // calculates pyramid
                                                           // levels etc to fill
                                                           // the
  // keyframe data structures with everything that's needed by the tracker..
  void MakeKeyFrame_Rest();  // ... while this calculates the rest of the data
                             // which the mapmaker needs.
...
typedef std::map<MapPoint*, Measurement>::iterator
