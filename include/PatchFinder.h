...
class PatchFinder {
 public:
  ...
  // transformation.
  int CalcSearchLevelAndWarpMatrix(MapPoint& p,
                                   SE3<> se3CFromW,
  ...
  void MakeTemplateCoarseCont(MapPoint& p);  // If the warping matrix has
                                             // already been pre-calced, use
                                             // this.
  void MakeTemplateCoarse(
      MapPoint& p,
      SE3<> se3CFromW,
      Matrix<2>& m2CamDerivs);  // This also calculates the warp.
  void MakeTemplateCoarseNoWarp(
      MapPoint& p);  // Identity warp: just copies pixels from the source KF.
  void MakeTemplateCoarseNoWarp(KeyFrame& k,
                                int nLevel,
                                CVD::ImageRef irLevelPos);  // Identity warp if
                                                            // no MapPoint
                                                            // struct exists
                                                            // yet.
  ...
  int ZMSSDAtPoint(
      ...
      const CVD::ImageRef& ir);  // This evaluates the score at one location
  // Results from step 3:
  ...
  // Step 4
  void MakeSubPixTemplate();  // Generate the inverse composition template and
                              // jacobians

  // Step 5 Functions
  bool IterateSubPixToConvergence(
      KeyFrame& kf,
      int nMaxIts);  // Run inverse composition till convergence
  double IterateSubPix(KeyFrame& kf);  // Single iteration of IC. Returns
                                       // sum-squared pixel update dist, or
                                       // negative if out of imag
  ...
  inline Matrix<2> GetCov() { return LevelScale(mnSearchLevel) * Identity; };
 ...
 protected:
  ...
  MapPoint* mpLastTemplateMapPoint;  // Which was the last map point this
                                     // PatchFinder used?
  Matrix<2> mm2LastWarpMatrix;       // What was the last warp matrix this
                                     // PatchFinder used?
};
